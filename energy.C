...
void calc(const argList& args, const Time& runTime, const fvMesh& mesh)
{
    ...
    scalar maxZ = analyticDict.lookupOrDefault("maxZ",-1);
    
    Info << " energy calculation " << endl; 
    Info << " From " << minZ << " [m] to " << ((maxZ == -1) ? 10000  : maxZ) << endl;
    ...
    forAll(mesh.C(),cellid) { 
		...
		if (maxZ != -1) { 
			InteriorSelector[cellid] *=(mesh.C()[cellid].component(2) < maxZ);
		}
    }    
...
//    dimensionedScalar Theta_m = sum(Tmean*mesh.V())/sum(mesh.V()); // Should read from file like the solver!!!. (chekc that this is what the solver does)
    ...
    dimensionedScalar small("f",var.dimensions(),scalar(1e-7));

    Info<< "Calculating alpha" << endl;
    volScalarField alpha("alpha", -InteriorSelector*beta*g.component(2)*pow((var+small),-1) );


//    Info << max(alpha) << " " << max( (InteriorSelector*pow(var+small,-1))->internalField() ) << " " << max(pow((var+small),-1))  << endl;
	...
	Info << " Total Volume " << sum(mesh.V()*InteriorSelector) << endl; 
	Info << " Total kinetic energy " << sum(KineticEnergy*mesh.V()*InteriorSelector) << " | Mean potential energy " << sum(PotentialEnergy*mesh.V())  << endl;
	Info << " Mean kinetic energy " << sum(KineticEnergy*mesh.V()*InteriorSelector)/sum(mesh.V()*InteriorSelector) << " | Mean potential energy " << sum(PotentialEnergy*mesh.V()*InteriorSelector)/sum(mesh.V()*InteriorSelector)  << endl;

	out  << runTime.timeName() << " " << sum(KineticEnergy*mesh.V()*InteriorSelector).value() << " " << sum(PotentialEnergy*mesh.V()*InteriorSelector).value() << " " << (sum(KineticEnergy*mesh.V()*InteriorSelector)/sum(mesh.V()*InteriorSelector)).value() << " " << (sum(PotentialEnergy*mesh.V()*InteriorSelector)/sum(mesh.V()*InteriorSelector)).value() << std::endl;  
